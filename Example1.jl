...
#     CSV.write(joinpath(tableFolder,"Ex1AvrgPrct"*string(ℓ)*".csv"),tb)
#     tb = DataFrame(A=Nvec,B=Avrg[ℓ,:])
#     CSV.write(joinpath(tableFolder,"Ex1AvrgAvrg"*string(ℓ)*".csv"),tb)
# end
...
# @everywhere function process_data(m, ℓ, κ, Nvec, dvec, σ, σ_out, vecNbr, jmp, SampleNbr, len_N, len_d, SuppErr, hErr)
#     d = dvec[m]
#     true_γplus = σ^2*(1+sqrt(d))^2
#     true_γmin = σ^2*(1-sqrt(d))^2
#     MP_h = x -> true_γmin < x < true_γplus ? 1/(2*π*d*x*σ^2) : 0
...
# @everywhere function parallel_processing(Nvec, dvec, σ, σ_out, vecNbr, jmp, SampleNbr, len_N, len_d)
#     SuppErr = zeros(Float64, len_d, SampleNbr, len_N)
#     hErr = zeros(Float64, len_d, SampleNbr, len_N)

#     @distributed for m = 1:len_d
#         @distributed for ℓ = 1:len_N
#             @distributed for κ = 1:SampleNbr
#                 process_data(m, ℓ, κ, Nvec, dvec, σ, σ_out, vecNbr, jmp, SampleNbr, len_N, len_d, SuppErr, hErr)
#             end
#         end
#     end
#     return SuppErr, hErr
...
# addprocs(160)
# SuppErr, hErr = parallel_processing(Nvec, dvec, σ, σ_out, vecNbr, jmp, SampleNbr, len_N, len_d)
...
# savefig(p2, joinpath(imgFolder, "Ex1Fig5.png"))
# p3 = plot(Nvec, (hErr1[checkpt] * (Nvec[checkpt])^(1/2)) * (Nvec).^(-1/2), color=:orange, linewidth=3, label=L"\mathrm{O}(N^{-1/2})", xlabel="N", ylabel=latexstring("\\text{Errors in } \\hat{h}"), legend=:topright, yscale=:log10, linestyle=:dash, framestyle=:box)
# p3 = plot!(Nvec, hErr1, yerror=hErr1_std, color=:red, linewidth=2, label="")
# p3 = scatter!(Nvec, hErr1, markersize=4, color=:red, marker=:diamond, label="d=" * string(dvec[1]))
# p3 = plot!(Nvec, hErr2, yerror=hErr2_std, color=:blue, linewidth=2, label="")
# p3 = scatter!(Nvec, hErr2, markersize=4, color=:blue, marker=:square, label="d=" * string(dvec[2]))
# p3 = plot!(Nvec, hErr3, yerror=hErr3_std, color=:green, linewidth=2, label="")
# p3 = scatter!(Nvec, hErr3, markersize=4, color=:green, marker=:circ, label="d=" * string(dvec[3]))
# savefig(p3, joinpath(imgFolder, "Ex1Fig6.png"))
