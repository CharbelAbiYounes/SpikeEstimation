...
for ℓ = 1:len_d
    ...
    for j=1:lenN
        ...
        σvec[1:3] = σ_out
        sqrtΣ = Diagonal(sqrt.(σvec))
        @threads for ℓ = 1:SampleNbr
            X = randn(N,M)
            W = sqrtΣ*(1/M*X*X')*sqrtΣ'|>Symmetric
            tmp = @timed begin
                evals = eigvals(W)
            end
            EigTime[ℓ,j]+=tmp.time
            tmp = @timed begin
                TChol,L_list = CholList(W,tol,k,jmp,max_iter,vecNbr)
                Nbr, Loc = EstimSpike(TChol,N,c=1.0)
            end
            LanTime[ℓ,j]+=tmp.time
            Sum += Nbr
            if Nbr==3
                count+=1
            end
        ...
        EigTime[ℓ,j] = EigTime[ℓ,j]/SampleNbr
...
savefig(p4,joinpath(imgFolder, "Ex1AvrgFig4.png"))
p5 = plot(Nvec,EigTime[1,:],color=:red,linewidth=2,label="",xlabel="N",ylabel="Time (in seconds)",legend=:topleft,framestyle=:box)
p5 = scatter!(Nvec, EigTime[1,:], markersize=4, color=:red, marker=:diamond, label="Eigenvalue Computation")
p5 = plot!(Nvec,Time[1,:],color=:orange,linewidth=2,label="")
p5 = scatter!(Nvec, Time[1,:], markersize=4, color=:orange, marker=:square, label="Lanczos Approach")
savefig(p5,joinpath(imgFolder, "Ex1Time1.png"))
p6 = plot(Nvec,EigTime[2,:],color=:red,linewidth=2,label="",xlabel="N",ylabel="Time (in seconds)",legend=:topleft,framestyle=:box)
p6 = scatter!(Nvec, EigTime[2,:], markersize=4, color=:red, marker=:diamond, label="Eigenvalue Computation")
p6 = plot!(Nvec,Time[2,:],color=:orange,linewidth=2,label="")
p6 = scatter!(Nvec, Time[2,:], markersize=4, color=:orange, marker=:square, label="Lanczos Approach")
savefig(p6,joinpath(imgFolder, "Ex1Time2.png"))
p7 = plot(Nvec,EigTime[3,:],color=:red,linewidth=2,label="",xlabel="N",ylabel="Time (in seconds)",legend=:topleft,framestyle=:box)
p7 = scatter!(Nvec, EigTime[3,:], markersize=4, color=:red, marker=:diamond, label="Eigenvalue Computation")
p7 = plot!(Nvec,Time[3,:],color=:orange,linewidth=2,label="")
p7 = scatter!(Nvec, Time[3,:], markersize=4, color=:orange, marker=:square, label="Lanczos Approach")
savefig(p7,joinpath(imgFolder, "Ex1Time3.png"))
# end
